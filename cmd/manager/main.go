...
func main() {
	...
	err = leader.Become(ctx, "cp4foo-operator-lock")
	if err != nil {
		log.Error(err, "")
		os.Exit(1)
	}
	...
	log.Info("start creating OperandRequest CR")
	if err = bootstrap.InitResources(mgr); err != nil {
		log.Error(err, "InitResources failed.")
		os.Exit(1)
	}
	log.Info("finish creating OperandRequest CR")
